                        ...
                        gboolean            check_only)
{
  ...
  if (META_WINDOW_TILED_MAXIMIZED (window))
  ...
  else if (META_WINDOW_MAXIMIZED (window))
    {
      ...
      /* Apply tiling gaps to maximized windows if enabled - use cached values */
      if (meta_prefs_get_tiling_gaps_enabled ())
        {
          ...
          target_size.width -= (outer_gap << 1);  /* Faster than 2 * outer_gap */
          target_size.height -= (outer_gap << 1);
        }
    }
  else
    {
